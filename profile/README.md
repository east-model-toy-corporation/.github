# 🏭 East Model Toy Corporation

> 找到你要的工具，避免重複造輪子。  
> 更新時間：2026-09-12 05:17 UTC（每日自動同步）  
> 📚 [完整知識庫 →](https://github.com/east-model-toy-corporation/p000-Automation-Intelligence-Bureau)

---

## 📦 p000

| REPO | 說明 | 部門 | 狀態 | Wiki |
|------|------|------|------|------|
| [p000-Automation-Intelligence-Bureau](https://github.com/east-model-toy-corporation/p000-Automation-Intelligence-Bureau) | 全公司自動化專案情報局 | — | ❓ | — |

## 📦 p011 · Gmail自動抓單

| REPO | 說明 | 部門 | 狀態 | Wiki |
|------|------|------|------|------|
| [p011-batch-product-editor](https://github.com/east-model-toy-corporation/p011-batch-product-editor) | 批次上架編輯器 | — | ❓ | — |
| [p011-super_crawler](https://github.com/east-model-toy-corporation/p011-super_crawler) | — | — | ❓ | — |
| [p011-waca-post-adjuster](https://github.com/east-model-toy-corporation/p011-waca-post-adjuster) | WACA 上架後快速修改商店頁設定 | — | ❓ | — |

## 📦 p014 · 91APP-IP分類同步GAS

| REPO | 說明 | 部門 | 狀態 | Wiki |
|------|------|------|------|------|
| [p014-elt-query-api](https://github.com/east-model-toy-corporation/p014-elt-query-api) | Google Sheet 去資料庫撈資料、回寫 | — | ❓ | — |
| [p014-gas-db-scripts](https://github.com/east-model-toy-corporation/p014-gas-db-scripts) | 資料庫互動、寫入相關 google app script | — | ❓ | — |
| [p014-performance_report](https://github.com/east-model-toy-corporation/p014-performance_report) | 報表 | — | ❓ | — |

## 📦 p021 · 全自動會員資料轉檔與建檔

| REPO | 說明 | 部門 | 狀態 | Wiki |
|------|------|------|------|------|
| [p021-ehobby-ec-auto](https://github.com/east-model-toy-corporation/p021-ehobby-ec-auto) | 關於電商平台的會員、訂單資料自動下載 | — | ❓ | — |

## 📦 p028

| REPO | 說明 | 部門 | 狀態 | Wiki |
|------|------|------|------|------|
| [p028-ehobby-wiki](https://github.com/east-model-toy-corporation/p028-ehobby-wiki) | Karpathy 知識庫wiki | — | ❓ | — |
| [p028-ehobby-wiki-vault](https://github.com/east-model-toy-corporation/p028-ehobby-wiki-vault) | 知識庫本庫(唯讀) | — | ❓ | — |

## 📦 p030

| REPO | 說明 | 部門 | 狀態 | Wiki |
|------|------|------|------|------|
| [p030-webtools-auto-pick](https://github.com/east-model-toy-corporation/p030-webtools-auto-pick) | WebTools (1)第一支每日依照年份順序匹配訂單; (2)第二支選昨日到今日匹配的蝦皮訂單取消撿貨返回 | — | ❓ | — |

## 📦 p031

| REPO | 說明 | 部門 | 狀態 | Wiki |
|------|------|------|------|------|
| [p031-beyblade-counters](https://github.com/east-model-toy-corporation/p031-beyblade-counters) | 戰鬥陀螺專案配貨控量 | — | ❓ | — |

## 📦 p07

| REPO | 說明 | 部門 | 狀態 | Wiki |
|------|------|------|------|------|
| [p07-store-auto-dispatch](https://github.com/east-model-toy-corporation/p07-store-auto-dispatch) | 門市補貨自動配貨、回寫、產出GD和ERP要匯入的excel | — | ❓ | — |

## 📦 p13

| REPO | 說明 | 部門 | 狀態 | Wiki |
|------|------|------|------|------|
| [p13-deadstock-carousel](https://github.com/east-model-toy-corporation/p13-deadstock-carousel) | 滯銷品大風吹 | — | ❓ | — |
| [p13-inventory-carousel](https://github.com/east-model-toy-corporation/p13-inventory-carousel) | 庫存大風吹 | — | ❓ | — |

## 📦 p16

| REPO | 說明 | 部門 | 狀態 | Wiki |
|------|------|------|------|------|
| [p16-invoice-ocr-hub](https://github.com/east-model-toy-corporation/p16-invoice-ocr-hub) | 進貨單OCR辨識，與資料庫匹配後產出GD可用的商品建檔與進貨單 | — | ❓ | — |

## 📦 p18

| REPO | 說明 | 部門 | 狀態 | Wiki |
|------|------|------|------|------|
| [p18-redirect-to-91app](https://github.com/east-model-toy-corporation/p18-redirect-to-91app) | 18-商品內頁導流91APP連結 | — | ❓ | — |

## 📦 p21

| REPO | 說明 | 部門 | 狀態 | Wiki |
|------|------|------|------|------|
| [p21-app91-member-automation](https://github.com/east-model-toy-corporation/p21-app91-member-automation) | 開發一支基於 Python 的自動化程式 (RPA)，執行每日 91APP 後台會員資料匯出，並完成資料轉檔、雲端備份以及回傳更新作業。 | — | ❓ | — |
| [p21-app91-order-automation](https://github.com/east-model-toy-corporation/p21-app91-order-automation) | 全自動下載91APP訂單上傳至資料庫 | — | ❓ | — |

## ❓ 未分類

> 以下 REPO 尚未設定 `dept-xxx` Topic，請協助補上。

| REPO | 最後推送 |
|------|----------|
| [.github](https://github.com/east-model-toy-corporation/.github) | 2026-09-11 |
| [91appmenber](https://github.com/east-model-toy-corporation/91appmenber) | 2026-04-20 |
| [Order-Cancellation](https://github.com/east-model-toy-corporation/Order-Cancellation) | 2026-09-09 |
| [Order-Database-91APP](https://github.com/east-model-toy-corporation/Order-Database-91APP) | 2026-08-13 |
| [Order-Database-PayNow](https://github.com/east-model-toy-corporation/Order-Database-PayNow) | 2026-06-29 |
| [Order-Database-WACA](https://github.com/east-model-toy-corporation/Order-Database-WACA) | 2026-08-13 |
| [Order-oversize-ehobby](https://github.com/east-model-toy-corporation/Order-oversize-ehobby) | 2026-08-14 |
| [Order-oversize-garage](https://github.com/east-model-toy-corporation/Order-oversize-garage) | 2026-05-19 |
| [Search-WACA-Orders-Excel](https://github.com/east-model-toy-corporation/Search-WACA-Orders-Excel) | 2026-08-19 |
| [UL-Evolution-Version](https://github.com/east-model-toy-corporation/UL-Evolution-Version) | 2026-09-07 |
| [c01-RefundAdmin](https://github.com/east-model-toy-corporation/c01-RefundAdmin) | 2026-08-05 |
| [c02-paynow-daily](https://github.com/east-model-toy-corporation/c02-paynow-daily) | 2026-06-24 |
| [c03-cancel-orders](https://github.com/east-model-toy-corporation/c03-cancel-orders) | 2026-09-07 |
| [c04-ServiceMail_AutoReply](https://github.com/east-model-toy-corporation/c04-ServiceMail_AutoReply) | 2026-07-18 |
| [c05-Order-Shipment-Notice](https://github.com/east-model-toy-corporation/c05-Order-Shipment-Notice) | 2026-08-20 |
| [database-elt](https://github.com/east-model-toy-corporation/database-elt) | 2026-08-21 |
| [ec-data-ssot](https://github.com/east-model-toy-corporation/ec-data-ssot) | 2026-04-27 |
| [ehobby-oms](https://github.com/east-model-toy-corporation/ehobby-oms) | 2026-08-17 |
| [ehobby-wms-3pl](https://github.com/east-model-toy-corporation/ehobby-wms-3pl) | 2026-09-08 |
| [line-Information-transformation-engine](https://github.com/east-model-toy-corporation/line-Information-transformation-engine) | 2026-04-14 |
| [store-order-consent](https://github.com/east-model-toy-corporation/store-order-consent) | 2026-06-04 |
| [vendor_order_parser](https://github.com/east-model-toy-corporation/vendor_order_parser) | 2026-09-11 |
